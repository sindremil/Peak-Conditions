...
function useActivePoint(destination: string, defaultValue: number = 0): [number, (newValue: number) => void] {
  ...
  const initialActivePoint = Number(sessionStorage.getItem(storageKey)) || defaultValue;
